...
def compute_distances_two_loops(x_train, x_test):
  ...
  for i in range(num_train):
    x_train_point = x_train[i].view(-1)
    for j in range(num_test):
      x_test_point = x_test[j].view(-1)
      dists[i][j] = torch.sum((x_train_point - x_test_point) ** 2)
  ##############################################################################
  #                             END OF YOUR CODE                               #
  ##############################################################################
  return dists
...
def compute_distances_one_loop(x_train, x_test):
  ...
  for i in range(num_train):
      dists[i] = torch.sum((x_test_flat - x_train_flat[i]) ** 2, dim=1)
  ##############################################################################
  #                             END OF YOUR CODE                               #
  ##############################################################################
  return dists
...
def compute_distances_no_loops(x_train, x_test):
  ...
  x_train_flat = x_train.view(num_train, -1)
  x_test_flat = x_test.view(num_test, -1)
  ab = torch.mm(x_train_flat, x_test_flat.t()) # (num_train, num_test)
  a2 = torch.sum(x_train_flat ** 2, dim=1, keepdim=True) # (num_train, 1)
  b2 = torch.sum(x_test_flat ** 2, dim=1) # (num_test,)
  dists = -2*ab + a2 + b2
  ##############################################################################
  #                             END OF YOUR CODE                               #
  ##############################################################################
